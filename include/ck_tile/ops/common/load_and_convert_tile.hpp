...
CK_TILE_DEVICE void load_and_convert_tile(WarpTile& dst, const WarpWindow& src)
{
    ...
    {
        ConverterLoader<DstDataType, UnaryOpSize, LoadTranspose>::load_interleaved_pk_type(dst,
                                                                                           src);
    }
    else
    {
        ConverterLoader<DstDataType, UnaryOpSize, LoadTranspose>::template load_with_type_convert<
            SrcDataType>(dst, src);
    }
}
} // namespace ck_tile
