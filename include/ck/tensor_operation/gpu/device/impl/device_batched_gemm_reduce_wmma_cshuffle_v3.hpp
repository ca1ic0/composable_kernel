        ...
        const ComputePtrOffsetOfStridedBatch compute_ptr_offset_of_batch)
{
                    ...
                    std::is_same_v<e_data_type, ck::bhalf_t>)))
    {
        ...
        auto epilogue_args =
            EpilogueType(reduces_batch, reduce_in_element_ops, reduce_out_element_ops, karg.M);
  ...
  public:
    ...
    struct Invoker : public BaseInvoker
    {
        float Run(const Argument& arg, const StreamConfig& stream_config = StreamConfig{})
        {
            ...
            const TailNumber tail_num        = GridwiseGemm::CalculateKBlockLoopTailNum(arg.KRaw_);
            ...
            if(has_main_k_block_loop)
            {
                ...
                {
                    if(tail_num == TailNumber::Full)
                    {
                        const auto kernel = kernel_batched_gemm_reduce_wmma_cshuffle_v3<
                            GridwiseGemm,
                            ReduceTrait,
            ...
            else
            {
                ...
                else if constexpr(BlkGemmPipelineVer == BlockGemmPipelineVersion::v3)
                {
                    ...
                    else if(tail_num == TailNumber::Odd)
                    {
                        const auto kernel = kernel_batched_gemm_reduce_wmma_cshuffle_v3<
                            GridwiseGemm,
                            ReduceTrait,
